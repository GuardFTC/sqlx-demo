// Package mysql @Author:冯铁城 [17615007230@163.com] 2025-07-15 19:04:09
package mysql
...
func Update() {
	...
	//3.在事务内更新数据
	tx := Db.MustBegin()
	exec, err := tx.NamedExec("update students set age = :age where id = :id", student)
	if err != nil {
		log.Fatal(err)
	}
	tx.Commit()
	...
	/*-----------------------------------------根据条件更新---------------------------------------------------*/
	//1.构建IN条件
	query, args, err := sqlx.In("UPDATE students SET name = CONCAT(name, '_update') WHERE age IN (?) AND id >= ?", []int{17, 18, 19}, 1)
	if err != nil {
		log.Fatal(err)
	}
